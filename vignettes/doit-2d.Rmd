...
vignette: >
  %\VignetteIndexEntry{doit-2d-example}
...
We use the DoIt method to approximate the banana-shaped posterior used in
Joseph (2012):
...
Here we plot the new approximation and highlight the new design points added in
the second wave:
...
ggplot(mapping=aes(x=theta1, y=theta2)) +
  ...
  geom_point(data=design %>% mutate(wave=rep(c('first','second'),c(n()-n_new, n_new))),
             mapping=aes(pch=wave, colour=wave), cex=2) + 
  scale_fill_viridis()
```
...
We use the functions `doit_expectation()` and `doit_variance()` to approximate posterior expectation and covariance matrix:
